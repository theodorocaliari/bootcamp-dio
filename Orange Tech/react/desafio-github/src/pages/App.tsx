...
import { IItemRepo } from "../components/ItemRepo/types";
...
function App() {
  ...
  const [repos, setRepos] = useState<string[]>([]); //type array of strings for useState

  const handleSearchRepo = async () => {
    const { data } = await api.get(`repos/${(!currentRepo) ? "pablohdev/dio-clone" : currentRepo}`);
  ...
  return (
    <Container>
      ...
      <Input value={currentRepo} onChange={changeValue} />
      <ButtonContainer onClick={handleSearchRepo}>
        Pesquisar
      </ButtonContainer>
